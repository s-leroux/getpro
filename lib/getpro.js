...
const { resolve: urlResolve } = require("url"); // legacy Node API
...
class Response extends Readable {
  ...
  get data() {
    if (this._done) {
      ...
      this._node_res.on('error', (err) => { this._done = true; reject(err); });
      this._node_res.on('data', (chunk) => { chunks.push(chunk); });
      this._node_res.on('end', () => {
        this._done = true;
        if (chunks.length > 0) {
  ...
  pipe(destination, options) {
    return this._node_res.pipe(destination, options);
  }
   ...
   */
  _read(size) {
    return this._node_res.read(size);
  }
}
...
class Request extends Writable {
  ...
  request(url, options) {
    ...
    const req = this._node_req = protocol.request(url, options, (res) => {
